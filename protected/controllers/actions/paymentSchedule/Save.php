...
class Save extends OAApiAction
{
	...
	public function run()
	{
		...
		else
		{
			...
			{
				echo json_encode( $this->formatError( 'Unauthorized external account.' . 'external_account_id = ' . $payment_schedule_external_account_id . ' and payment_schedule_id = ' . $payment_schedule_id ) );
				Yii::app()->end();
			}
			...
			$paymentSchedule->payment_schedule_external_account_id = $payment_schedule_external_account_id;
